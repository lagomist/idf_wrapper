...
static const uint8_t default_cacert_pem[] = {\
"-----BEGIN CERTIFICATE-----\n"\
...
{
    TY_LOGI("Tuya mqtt disconnect");
}

static void tuya_app_task(void *pvParameters)
{
    int ret = OPRT_OK;

    tuya_mqtt_context_t *client = &client_instance;

    // wait wifi connecterd
                            ...
                            tuya_connect_callback_t connect_callback)
{
    ...
    const tuya_mqtt_config_t config = {
        .host = "m2.tuyacn.com",
        .port = 8883,
        .cacert = default_cacert_pem,
        .cacert_len = sizeof(default_cacert_pem),
        .device_id = device.id,
        ...
        .on_messages = messages_callback,
    };

    ret = tuya_mqtt_init(&client_instance, &config);
    if (ret != OPRT_OK) {
        TY_LOGE("tuya_mqtt_init failed!");
        return ret;
    }

    xTaskCreate(tuya_app_task, "tuya_app_task", 1024 * 4, NULL, 5, NULL);
